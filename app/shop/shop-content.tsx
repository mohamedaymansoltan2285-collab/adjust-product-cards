...
import { CATEGORIES, SUB_CATEGORIES, COLORS, type Product } from "@/lib/types"
import {
  SlidersHorizontal,
  LayoutGrid,
  List,
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (selectedColors.length > 0) {
        productsData = productsData.filter((p) => p.colors?.some((c) => selectedColors.includes(c)))
      }

      productsData = productsData.filter((p) => {
        const price = p.discount > 0 ? p.discountedPrice : p.salePrice
              ...
              <div className={cn("col-span-1", showFilters ? "lg:col-span-3" : "lg:col-span-4")}>
                {loading ? (
                  ...
                  <div className="text-center py-20">
                    ...
                    <h3 className="text-xl font-semibold mb-2">{t("لا توجد منتجات", "No Products Found")}</h3>
                    <p className="text-muted-foreground">{t("جرب تغيير الفلاتر", "Try adjusting your filters")}</p>
                  </div>
                ) : viewMode === "grid" ? (
